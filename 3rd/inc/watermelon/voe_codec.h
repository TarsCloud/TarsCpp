...
class WEBRTC_DLLEXPORT VoEDecodeProcess {
 public:
  // This method is called when a new incoming frame is prepared for decoded.
  // if return true, will interrupt next processes.
  virtual void OnDecode (const int channel, char payload,
                         unsigned time_stamp, bool marked,
                         const char* data, int len) = 0;
 protected:
  virtual ~VoEDecodeProcess() {}
};
...
public:
    ...
    // Registers an instance of a user implementation of the VoEDecodeProcess.
    virtual int RegisterDecodeProcess(const int channel,
                                    VoEDecodeProcess& process) = 0; 

    // Removes an already registered instance of VoEDecodeProcess.
    virtual int DeregisterDecodeProcess(const int channel) = 0;
    ...
    virtual int SetOpusMode(int channel, OpusMode mode) = 0;

    virtual int SetAudioArs(int channel, bool enable, bool inband = true) = 0;
    virtual int SetAudioArsParams(int channel, unsigned int bpsLow, unsigned int bpsHigh) = 0;
    virtual int GetCodecRateAndFrameLength(int channel,
                                             int &rateBps, int &frameLenMs) = 0;
