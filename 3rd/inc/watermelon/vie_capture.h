...
struct CaptureCapability {
  ...
  bool interlaced;
  ///aspectRatio <= 0: keep capture size,don't crop; else crop capture size to fit aspectRatio
  float aspectRatio;
  // fixed the capture config
  bool fixed;
  CaptureCapability() {
    width = 0;
    height = 0;
...
class WEBRTC_DLLEXPORT ViECaptureObserver {
 public:
  ...
  // This method is called if the capture size changed.
  virtual void InformActualCaptureSize(const int capture_id,
...
class WEBRTC_DLLEXPORT ViECapture {
 public:
  ...
  virtual int DisconnectCaptureDevice(const int video_channel, const int capture_id) = 0;
  ...
  virtual int SetCameraFps(const int capture_id, const WebRtc_Word32 maxFps)=0; 

  // Set Clipping
  virtual int SetClipping(const int capture_id, float& x, float& y,  float& scale, bool bInform = false) = 0;
  ...
  virtual int RegisterGeneralProcessObserver(const int capture_id,
      const int gd_instance_id, const int gd_stream_id,
      const int gd_track_id) = 0;

  virtual int DeRegisterGeneralProcessObserver(const int capture_id) = 0;
