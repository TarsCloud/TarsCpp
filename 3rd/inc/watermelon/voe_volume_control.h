...
public:
    ...
    // Gets speaker max volume, in real scale, not [0,255]
    virtual int MaxSpeakerVolume(unsigned int& volume) = 0;
    ...
    // Mutes the signal output to speaker completely without affecting
    // the audio device volume.
    virtual int SetOutputMute(int channel, bool enable) = 0;

    // Gets the current speaker output mute state.
    virtual int GetOutputMute(int channel, bool& enabled) = 0;

    //Set howl suppression enable/disable.
    virtual int SetHowlSuppress(bool enable) = 0;
    ...
    // Gets the microphone speech |pitch| in Hz
    virtual int GetSpeechInputPitch(unsigned int& pitch) = 0;
    ...
    // Sets a volume |scaling| applied to the input signal of a specific
    // channel. Valid scale range is [0.0, 10.0].
    virtual int SetChannelInputVolumeScaling(int channel, float scaling) = 0;

    // Gets the current volume scaling for a specified |channel|.
    virtual int GetChannelInputVolumeScaling(int channel, float& scaling) = 0;
