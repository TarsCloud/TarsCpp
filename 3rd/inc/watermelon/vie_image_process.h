...
class WEBRTC_DLLEXPORT ViEImageProcess {
 public:
  ...
  // This function registers a EffectFilter to use for the rendered video
  // stream on an record Capturing.
  virtual int RegisterCapturingRecordEffectFilter(const int capture_id,
                                       ViEEffectFilter& capturing_filter) = 0;

  // This function deregisters a Capturing effect filter for a specified capture.
  virtual int DeregisterCapturingRecordEffectFilter(const int capture_id) = 0; 

  // This function registers a EffectFilter to use for the rendered video
  // stream on an record Incoming.
  virtual int RegisterIncomingRecordEffectFilter(const int video_channel,
                                       ViEEffectFilter& send_filter) = 0;

  // This function deregisters a Incoming effect filter for a specified channel.
  virtual int DeregisterIncomingRecordEffectFilter(const int video_channel) = 0; 

  // This function registers a EffectFilter to use for the rendered video
  // stream on an record Outgoing.
  virtual int RegisterOutgoingRecordEffectFilter(const int video_channel,
                                       ViEEffectFilter& send_filter) = 0;

  // This function deregisters a Outgoing effect filter for a specified channel.
  virtual int DeregisterOutgoingRecordEffectFilter(const int video_channel) = 0; 
 protected:
  ViEImageProcess() {}
