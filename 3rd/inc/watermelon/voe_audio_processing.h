...
public:
    ...
    // Gets the noise level and SNR from NS module.
    virtual int GetNoiseLevel(float& noise_level, float& snr) = 0;
    ...
    virtual int SetEcInitSuppMode(EcInitSuppMode mode) = 0;

	// Sets the EC Self-Cancellation mode.
	virtual int SetEcSelfCancellationMode(EcSelfCancellationMode mode) = 0;

    // Sets the EC Single-talk Suppress mode.
    virtual int SetEcSingletalkSuppMode(EcSingletalkSuppMode mode) = 0;

    // Sets the EC NLP(non-linear processing) Suppress mode.
    virtual int SetEcNlpSuppLevel(EcNlpSuppLevel level) = 0;
    ...
    // Sets the conference status for a specified |channel| number.
    virtual int SetConfStatus(int channel, bool enable) = 0;

    // Gets the conference status for a specified |channel| number.
    virtual int GetConfStatus(int channel, bool& enabled) = 0;

    // Enables recording of AEC signal: far, near, out.
    virtual int StartAecRecording(OutStream* stream) = 0;

    // Disables recording of AEC.
    virtual int StopAecRecording() = 0;

    virtual int SetExtApmStatus(bool enable) = 0;
    virtual int GetExtApmStatus(bool& enable) = 0;
