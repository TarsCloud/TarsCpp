...
enum ViEPacketTimeout {
  NoPacket = 0,
  PacketReceived = 1,
  FirstPacketReceived = 2,
  NoRtcpPacket = 3,
  KeepAliveRPacketeceived = 4
};
...
class WEBRTC_DLLEXPORT ViENetworkObserver {
 public:
  ...
  // This method is called if network status is changed
  virtual void NetwStatusReport(const int video_channel,
    NetwBandwidthStatus recvType, NetwBandwidthStatus sendType) = 0;
 protected:
...
class WEBRTC_DLLEXPORT ViENetwork {
 public:
  ...
  // When using external transport for a channel, send RTP packets.
  virtual int SendRTPPacket(const int video_channel,
                            const void* data,
                            const int length) = 0;
  ...
  // set network limit for testing
  virtual int SetLimit(int video_channel,
