...
int TC_HttpRequest::sendNetBuffer(TC_NetWorkBuffer &in, TC_TCPClient &tcpClient)
{
	//buf不为空,先发送buffer的内容
	while (!in.empty())
	{
		auto data = in.getBufferPointer();
		assert(data.first != NULL && data.second != 0);

		int iRet = tcpClient.send(data.first, (uint32_t)data.second);
...
//		static thread_local shared_ptr<TC_OpenSSL::CTX> ctx = TC_OpenSSL::newCtx("", string(CMAKE_SOURCE_DIR) + "/certs/client.crt", string(CMAKE_SOURCE_DIR) + "/certs/client.key", false, "");
		...
		while (true)
		{
			auto recvBuffer = out.getOrCreateBuffer(5 * 1024, 16 * 1024);
	...
	{
		iRet = tcpClient.send(sSendBuffer.c_str(), sSendBuffer.length());
		if (iRet != TC_ClientSocket::EM_SUCCESS)
		{
			return iRet;
		}
