...
class WEBRTC_DLLEXPORT ViERenderObserver {
 public:
  virtual void ImageRotationChanged(const int renderId,const int width,
                              const int height,
                              const int rotation,
                              const bool backCamera) = 0;

  virtual void RenderFrameRate(const int renderId,
                              const unsigned int frame_rate) = 0;
  virtual void RenderDiscontinuity(const int renderId,
                                   const unsigned int disconTime,
                                   const unsigned int conTime) = 0;

 protected:
  virtual ~ViERenderObserver() {}
};
...
class WEBRTC_DLLEXPORT ViERender {
 public:
  ...
  // This function rotate rendered stream
  virtual int RotateRenderStream(const int render_id,
                                 const int angle) = 0;


  // This function get rendered stream rotation
  virtual int RenderStreamRotation(const int render_id) = 0;
  ...
  virtual int RegisterRenderObserver(const int render_id, ViERenderObserver& observer) = 0;

  virtual int DeregisterRenderObserver(const int render_id) = 0;

  virtual bool RenderIsExisted(const int render_id) = 0;

  virtual int SetRenderUniqueId(const int render_id, const char unique_idUTF8[512]) = 0;
