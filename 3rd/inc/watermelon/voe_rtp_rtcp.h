...
public:
    ...
    virtual void OnPttParamsReceived(const int channel, const WebRtc_UWord8 ptt_type, const WebRtc_UWord8 squ,
        const WebRtc_UWord8 id, const WebRtc_UWord8 pm, const WebRtc_UWord8 ptts,
        const WebRtc_UWord8 sct, const WebRtc_UWord16 extension_data) = 0;

    virtual void OnAudioPlayDurationReceived(const int channel, const WebRtc_UWord16 duration) = 0;
...
public:
    ...
    virtual int SetRTPAudioLevelIndicationStatus(
        int channel, bool enable, unsigned char ID = 1, bool packLevel = true) = 0;
    ...
    virtual int SetRTCPName(const int channel,
                           const unsigned char* name,
                           unsigned char len) = 0;
    ...
    // Gets our locally created statistics for a specific |channel|.
    virtual int GetReceivedRTCPStatistics(int channel,
    ...
    // Gets statistics reported by the remote client for a specific |channel|.
    virtual int GetSentRTCPStatistics(int channel,
                                      unsigned short& fraction_lost,
                                      unsigned int& cumulative_lost,
                                      unsigned int& average_lost,
                                      unsigned int& extended_max,
    ...
    // Gets statistics reported by the remote client for a specific |channel|.
    virtual int GetTotalDataCounters(int channel,
    ...
    virtual int SetNACKFECStatus(const int audio_channel, const bool enable) = 0;

    virtual int SetRTXReceiveStatus(const int audio_channel,
                                    const bool enable) = 0;
    virtual int SetRTXReceivePayloadType(const int audio_channel,
                                         const char rtxPayloadType,
                                         const char associatedPayloadType) = 0;
    virtual int SetRTXSendStatus(const int audio_channel,
                                 const bool enable) = 0;
    virtual int SetRTXSendPayloadType(const int audio_channel,
                                      const char rtxPayloadType,
                                      const char associatedPayloadType) = 0;

    // Sets the Forward Error Correction (FEC) status on a specific |channel|.
    virtual int SetFECStatus(
        int channel, FecMode mode, int redPayloadtype = -1) = 0;
    ...
    // Sets the anti loss through resend the same packets.
    virtual int SetAntiLossStatus(int channel, bool enable) = 0;
    virtual int GetRedNum(int channel, int& rednum) = 0;
    ...
    virtual int GetRtpSocket(int channel) = 0;
    virtual int GetRtcpSocket(int channel) = 0;

    virtual int EnableRtcpMux(int channel, bool enable) = 0;
    virtual int ResetSsrcOnSeqWrap( int channel,bool enable) = 0;

    virtual int SetPttHeaderExtension(const int channel, bool enable) = 0;
    virtual int SetPttParams(const int channel, const unsigned int ptt_type, const unsigned int squ,
        const unsigned int id, const unsigned int pm, const unsigned int ptts, const unsigned int sct,
        const bool isExtension, const unsigned int extension_data, const unsigned int send_times) = 0;
    virtual int SetAudioPlayMark(const int channel, bool enable, void* ptrMarks, const unsigned int size) = 0;
protected:
    VoERTP_RTCP() {}
    virtual ~VoERTP_RTCP() {}
};
