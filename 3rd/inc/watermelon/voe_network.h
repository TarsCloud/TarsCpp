...
public:
    ...
    virtual int SendRTPPacket(const int channel,
                            const void* data,
                            const int length) = 0;
    ...
    virtual int SetPacketTimeoutNotification(
        int channel, bool enable, int timeoutSeconds = 2, int rtcpTimeoutSeconds = 0) = 0;
    ...
    virtual int SendUDPPacket(
        ...
        int& transmittedBytes, bool useRtcpSocket = false,
        WebRtc_UWord16 portnr=0,const char* ip=0) = 0;

    // set network limit for testing
    virtual int SetLimit(
        int channel, int delayMilliseconds, int jitterMilliseconds,
        int lostRatio, int bandwidthBps, int outOfOrder, int maxBufferLen,
