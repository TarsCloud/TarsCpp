...
class WEBRTC_DLLEXPORT VoEHardwareObserver
{
public:
    virtual void AudioDeviceInterruption(const bool active) = 0;
protected:
    virtual ~VoEHardwareObserver() {}
};
...
public:
    ...
    virtual int SetRecordingDevice(char strGuidUTF8[512],
    ...
    virtual int SetPlayoutDevice(char strGuidUTF8[512]) = 0;

    // Get the audio device used for playout.
    virtual int GetCurrentPlayoutDevice(char strGuidUTF8[512]) = 0;

    // Get the audio device used for recording.
    virtual int GetCurrentRecordingDevice(char strGuidUTF8[512]) = 0;
    ...
    virtual int SetAGC(bool enable) = 0;
    virtual bool AGC() const = 0;

    // Registers an instance of a user implementation of the VoEHardwareObserver
    virtual int RegisterObserver(VoEHardwareObserver& observer) = 0;

    // Removes an already registered instance of VoEHardwareObserver.
